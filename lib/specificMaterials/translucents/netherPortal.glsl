...
// Vanilla portal liquid appearance with golden particles
// Keep original portal liquid colors (purple/magenta) but reduce intensity
color.a = sqrt1(color.a) * 0.8;

emission *= emission;
emission = clamp(emission * 80.0, 0.02, 0.9) * 5.0;
...
#define PORTAL_REDUCE_CLOSEUP
    ...
    if (CheckInsideVoxelVolume(voxelPos)) {
        ...
        // Subtle portal edges - closer to vanilla
        vec3 goldGlow = vec3(1.1, 1.05, 0.2) * (0.3 + 0.1 * sin(frameTimeCounter * 2.0));
        color.rgb += goldGlow * smoothstep(0.0, 0.2, abs(newPos.x * newPos.y)) * 0.15;
        color = mix(color, edgeColor, edge);
        emission = mix(emission, 3.5, edge);
    }
#endif
